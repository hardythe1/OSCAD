...
\subsection{Hotkeys}\index{Hotkeys!Schematic Editor} 
\label{short}
...
Once all the components are placed, the schematic editor would look like the Figure \ref{afterplace}.
\begin{figure}
\centering
...
\label{anno}
\end{figure}
Let us now do ERC or Electric Rules Check. To do so, click on \textit{Perform electric rules check} tool from the top toolbar. Click on \textit{Test Erc} button. You may get an error as shown in Figure \ref{erc}. Click on close in the test erc window.\index{ERC}
\begin{figure}
\centering
